...
                ["stop"," Happy_Birthday"]
    ...
    if not __resIsSuccess(res):
        logging.error("error code = %d msg = %s",res.get("code",-1),res.get("msg",""))
        return []
    motions =  res["data"]["motions"]
    motionsName = []
    for motion in motions:
        motionName = motion.get("name")
        if motionName:
            motionsName.append(motionName)
    return motionsName
...
class GamepadKey(Enum):
    ...
    L_STICK_X = "key_L_STICK_X"
    L_STICK_Y = "key_L_STICK_Y"
    R_STICK_X = "key_R_STICK_X"
    R_STICK_Y = "key_R_STICK_Y"
    BT = "key_BT"
    START = "key_START"
    POWER = "key_POWER"
    RESERVE = "key_RESERVE"
